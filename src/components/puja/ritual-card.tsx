...
export const RitualCard: React.FC<RitualCardProps> = ({ ritual, className }) => {
  ...
  return (
    <div
      className={cn(
        'agomoni-card p-4 sm:p-8 relative overflow-hidden flex flex-col justify-between border-2 border-[#E7C878]/30 bg-gradient-to-b from-[#1C120F] to-[#120B09] shadow-2xl space-y-4 sm:space-y-6 hover:border-[#E7C878]/50 transition-all duration-300',
        className
      )}
    >
      {/* 1. Header: Timing Badge & Tithi */}
      <div className="flex items-center justify-between gap-2 pb-2.5 sm:pb-3 border-b border-[#FFFDF8]/10">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-[#120B09] border border-[#E7C878]/40 text-xs sm:text-sm text-[#E7C878] font-mono font-bold shadow-xs">
          <Clock className="w-3.5 h-3.5 text-[#E7C878]" />
          <span>{language === 'bn' ? ritual.timeBn : ritual.timeEn}</span>
        </div>

        <span className="text-[11px] sm:text-xs text-[#E7C878]/80 uppercase tracking-wider font-semibold font-mono px-2.5 sm:px-3 py-1 rounded-full bg-[#FFFDF8]/[0.05] border border-[#FFFDF8]/10">
      ...
      <div className="space-y-1.5 sm:space-y-2">
        <h3 className="text-lg sm:text-2xl font-bold font-serif text-[#FFF8EA] leading-snug tracking-normal">
      ...
      <div className="p-3.5 sm:p-5 rounded-[18px] sm:rounded-[22px] bg-[#FFFDF8]/[0.04] border border-[#FFFDF8]/10 space-y-1 sm:space-y-1.5 shadow-xs">
        ...
        <p className="text-xs sm:text-sm text-[#FFF8EA]/95 leading-relaxed font-sans pt-1">
      ...
      {ritual.mantraPreviewBn && (
        <div className="p-4 sm:p-5 rounded-[18px] sm:rounded-[22px] bg-[#120B09] border-2 border-[#E7C878]/35 text-center space-y-1.5 sm:space-y-2 shadow-inner">
          <div className="flex items-center justify-center gap-1.5 text-[10px] sm:text-[11px] text-[#E7C878] font-bold uppercase tracking-widest font-mono">
            <Scroll className="w-3.5 h-3.5 text-[#E7C878]" />
            <span>{language === 'bn' ? 'সংশ্লিষ্ট শাস্ত্রীয় মন্ত্র' : 'Sacred Shloka / Mantra'}</span>
          </div>
          <p className="text-xs sm:text-base font-mono text-[#E7C878] font-bold leading-relaxed">
      ...
      {ritual.significanceBn && (
        <div className="p-3.5 sm:p-4 rounded-[16px] sm:rounded-[18px] bg-[#FFFDF8]/[0.04] border border-[#E7C878]/25 flex items-center gap-2 text-xs sm:text-sm text-[#FFF8EA]/90 font-medium">
        ...
        <div className="p-3.5 sm:p-5 rounded-[18px] sm:rounded-[22px] bg-[#FFFDF8]/[0.04] border border-[#FFFDF8]/10 space-y-2.5 sm:space-y-3 shadow-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5">
            {(language === 'bn' ? ritual.itemsNeededBn : ritual.itemsNeededEn).map((item, idx) => (
              <div
                key={idx}
                className="p-2.5 sm:p-3 rounded-[14px] sm:rounded-[16px] bg-[#120B09]/90 border border-[#E7C878]/25 hover:border-[#E7C878]/50 transition-all flex items-center gap-2.5 sm:gap-3 shadow-2xs"
              >
                <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-[#E7C878]/15 text-[#E7C878] text-[11px] sm:text-xs font-mono font-bold flex items-center justify-center flex-shrink-0">
                  {idx + 1}
                </span>
